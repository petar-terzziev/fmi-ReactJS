...
class Subcategory extends React.Component {
  ...
  loadthread = () =>{console.log("yeye")};

  render() {
    const userActions = (
      <div>
        <Button variant="contained" color="primary" onClick={this.newThread}>New thread</Button>
      </div>
    );
    ...
    return (
      ...
      <div className={classes.paper}>
          <Typography component="h1" variant="h5" color="primary" className={classes.title}>{this.state.title}</Typography>
       
          <Divider />
          <List>
          {this.state.threads && this.state.threads.map((thread, index) => (
                 <ListItem button component={Link} to={`/thread/${thread.id}`}>
